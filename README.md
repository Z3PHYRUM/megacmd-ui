...
| `DATA_DIR` | `/data` | Path inside the container for `queue.json`, `activity.log`, `settings.json`, `bookmarks.json`, and `postConfirmWatches.json` |
...
The folder file picker currently still has to ask MEGAcmd to actually enqueue the folder's contents to discover what's inside (each file is individually paused the instant it's discovered, so nothing downloads before you confirm — but MEGAcmd still has to enumerate every file). For folders with hundreds of subfolders, discovery can take a while, and the picker gives up waiting after 30 seconds — if that happens, the file list may be incomplete. This is safe either way: only the files you explicitly select are ever resumed. "Download Selected" responds immediately regardless of how many files you deselected — cleaning up everything else you didn't pick (potentially thousands of individual cancel commands for a huge folder) happens in the background over the following few minutes rather than blocking the button, so a "🧹 Cleaning up ~N leftover transfers..." banner appears under the transfer table until it's done, and the count may keep dropping for a while after the picker closes rather than updating instantly. Internal discovery/cleanup queries look across up to 5000 transfers (separate from the 1000-row display cap on the main table), so a folder in the low thousands of files should be fully seen and tracked. If you hit an incomplete list, re-browsing the same folder link again (or picking it up from Bookmarks) will show whatever wasn't caught the first time.

A pure listing command that never touches the transfer queue at all (which would remove this limitation entirely) isn't available: `mega-ls -R` on a public folder link fails with "Couldn't find" on MEGAcmd 2.5.2.1, confirmed against a link known to work fine with `mega-get`. `mega-transfers -c/-p/-r` also only accept a single tag per call (confirmed via `mega-transfers --help` — no batch/list syntax), so cleanup speed for a huge folder is bounded by one `docker exec` round-trip per cancelled file; concurrency (`runInBatches`' batch size in `server.js`) is the only available lever.

If the cleanup never seems to happen at all (unwanted transfers just sit `PAUSED` indefinitely instead of disappearing, with no cleanup banner ticking down), check `docker logs megacmd-ui` for `[BROWSE] postConfirmWatches sweep failed` — listing a very large transfer queue (`mega-transfers --limit=...`) can take longer than a generic command timeout would allow, silently failing the cleanup sweep every cycle. `mega-transfers` gets a 45s timeout specifically for this reason; if it's still not enough for your queue size, that's the number to raise (`EXEC_TIMEOUT` in `server.js`).

The list of "what to clean up" for an in-progress folder confirm is persisted to `postConfirmWatches.json` in `DATA_DIR`, so redeploying `megacmd-ui` mid-cleanup doesn't orphan it — on startup you should see `[BROWSE] resumed N watch(es) from a previous run` in the logs if one was still active. Without this, a redeploy during an active cleanup would silently abandon it, leaving paused transfers with nothing left to cancel them.
...
**Known gap:** the folder picker (see [Troubleshooting](#folder-picker-is-slow-or-incomplete-for-very-large-folders)) discovers a folder's contents by actually enqueueing it rather than a pure listing call. Confirmed on this MEGAcmd version: `mega-ls -R` cannot list a public folder link directly (fails with `Couldn't find`, tested against a link known to work with `mega-get`), so there's no listing-only alternative available without importing the folder into the account first (`mega-import`) — untested, would add something to the account, not implemented.
