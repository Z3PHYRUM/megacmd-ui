...
- **Non-blocking queueing** — every MEGA download is handed to the MEGAcmd daemon with `get -q` and tracked in the transfer table, so the Download button returns immediately instead of holding the request open for the length of the download; completion is detected by polling the transfer table, so folder-picker downloads get completion notifications too
- **Bandwidth quota handling** — MEGAcmd retries a quota-refused transfer natively (that's the `RETRYING` state in the table). Links that `mega-get` rejects outright still go to the app's own retry queue, retried every 15 minutes
...
The fallback enqueues via MEGAcmd's own `get -q` ("queue and return"), so the client hands the folder to the daemon and exits cleanly, and any error — including a quota refusal — comes back immediately rather than after discovery has already timed out. Earlier versions backgrounded the client with a shell `&` instead, which let Docker tear down the exec session and kill `mega-get` mid-enumeration.
